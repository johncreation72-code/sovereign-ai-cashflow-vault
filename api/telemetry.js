...
function getOnChainUSDTBalance(address) {
  return new Promise((resolve) => {
    ...
    // balanceOf signature: 0x70a08231 + 24 zeros + address without 0x
...
export default async function handler(req, res) {
  ...
  // Handle Inbound Event Beacon (POST)
  if (req.method === 'POST') {
    try {
      ...
      if (eventType === 'pageview') {
        ...
        telemetryStore.events.unshift({
          ...
          client: detail || "Tamper-Evident Contract Signed",
          status: "ENGAGED"
        });
      ...
      // Limit events to last 20
      if (telemetryStore.events.length > 20) {
        telemetryStore.events = telemetryStore.events.slice(0, 20);
      }
